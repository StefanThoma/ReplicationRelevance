...
```{r, include = TRUE}
scales.study@table <- relevance_table.study(scales.study, mem = TRUE)
scales.study@difference.table <- ReplicationRelevance::effect_differences.study(scales.study)
...
lobue.study <-  new("study", 
            name = "lobue", 
            manyLabs = "ml5",
            data.replication = as.data.frame(subset(lobue, protocol == "RP")),
            data.revised = as.data.frame(subset(lobue, protocol == "NP")),
...
Obviously, the original effect is still missing. Needs to be added ASAP!
