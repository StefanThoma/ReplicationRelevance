...
summary.study <- function(object){
  ...
  if(object@manyLabs == "ml5"){
    ...
    summ <- summ %>% select(type, all_of(cols.temp))
...
MixedModels.study <-function(object, methodFamily=NULL, transformation=NULL){
  ...
  if(tolower(methodFamily)!="gaussian"){
    ...
    fit0 <- glmer(formula=as.formula(paste0(yVar," ~", base_iv, "+ (1|Location)")), dat, family=methodFamily)
    fit1 <- glmer(formula=as.formula(paste0(yVar," ~", base_iv, "+ (1+", xVar[1] ,"|Location)")), dat, family=methodFamily)
  } else{
    fit.empty<-aov(formula=as.formula(paste0(yVar," ~", paste(base_iv, collapse = " + "))), dat, na.action=na.omit)
    fit0 <- lmer(formula=as.formula(paste0(yVar," ~", base_iv, "+ (1|Location)")), dat)
    if("sex" %in% xVar){ # We had errors at convergence: this is an ad hoc solution
      fit1 <- lmer(formula=as.formula(paste0(yVar," ~", base_iv, "+ (1+", xVar[1] ,"|Location)")), dat,
                   control = lmerControl(optimizer ="Nelder_Mead"))
    }
    else{fit1 <- lmer(formula=as.formula(paste0(yVar," ~", base_iv, "+ (1+", xVar[1] ,"|Location)")), dat)

    if(isSingular(fit1)){
      fit1 <- lmer(formula=as.formula(paste0(yVar," ~", xVar, " + (1 | Location) + (" , xVar[1] ," - 1 | Location)")), dat)
      sing <- isSingular(fit1)
    }
    }
  }
  ## get intraclass correlation (icc)
  icc <- as.data.frame(icc_specs(fit1))
  if(nrow(icc) == 4){
...
LRT.study <-function(object){
  ...
  if(is_empty(object@mixedModels)){
...
##'  This function calls the functions escalc_inputs_smd and escalc_inputs_or from
##'  the file replication-fn.R
...
relevance_table.study <- function(object,
  ...
  if(mem){
    ...
    after_escalc["All", c(names(stnd.fix),
                          ...
                          "Rls",
                          "Rlp")] <-  c(stnd.fix,
                                        nobs(object@mixedModels$Models$RandomCoefficients),
                                        stnd.fix["stcoef"]/object@relevance.threshold,
                                        stnd.fix["stciLow"]/object@relevance.threshold,
                                        stnd.fix["stciUp"]/object@relevance.threshold)
    }
...
plot_difference.study <- function(object, cutoff = NULL, ci.type = "wald", standardise = TRUE){
  if(!is.null(cutoff) & length(cutoff) == 1){cutoff <- c(-cutoff, cutoff)}
  if(is_empty(object@difference.table)){
    warning("the object does not contain the table. The function mixedModels.study is called and results are used")
...
plot.table.study <- function(ggtable, category, threshold){
  ggplot(data=ggtable, aes(y=as.numeric(Location), x=stcoef, xmin=stciLow, xmax=stciUp, col = category))+
    ...
    geom_point(alpha = ggtable$alp, size = ggtable$alp) +
    #adds the CIs
    geom_errorbarh(height=.1, alpha = ggtable$alp, size = ggtable$alp) +

    #this changes the features of the overall effects
    #one could resize, reshape, or recolor these points if desired
    geom_point(data=subset(ggtable, as.numeric(Location)==length(Location)), aes(y = as.numeric(Location), x=stcoef)) +
    ...
    scale_y_continuous(name = "", breaks=1:nrow(ggtable), labels = paste(ggtable$Location," (",category, ")",  sep = ""), trans="reverse")+
...
r_squared.study <- function(object,  coverage_prob = .95, bootstrap.type = "basic"){
  ...
  stat.f.lm <- function(dat, i){
    dat2 <- dat[i, ]
    m1 <- stats::glm(as.formula(formula1), data  = dat2, family = object@family)
    m2 <- stats::glm(as.formula(formula2), data = dat2, family = object@family)
  ...
  if(object@manyLabs=="ml5"){
    ...
    out.all <- c(round(c(MEMo_boot_ci$t0, sd(MEMo_boot$t), MEMo_boot_ci[[bootstrap.type]][4:5]), digits = digitsForRounding), nrow(object@data.revised), type = "fixef.rand.coef")
    out.rev <- cbind(round(for.all.labs(object@data.revised), digits = digitsForRounding), type = "revision")
    out.rep <- cbind(round(for.all.labs(object@data.replication), digits = digitsForRounding), type = "replication")
